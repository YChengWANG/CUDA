...
__global__ void convolutional_2D_constant_kernel(int *N, int *P, int Width){
    ...
    for(int row=0;row<Mask_Width;row++){
        for(int col=0;col<Mask_Width;col++){
            if(
                ...
                //printf("is in\n");
                //printf("%d, %d\n",N[(N_start_point_y + row) * Width + (N_start_point_x + col)], d_M[row * Mask_Width + col]);
                Pval += N[(N_start_point_y + row) * Width + (N_start_point_x + col)] * d_M[row * Mask_Width + col];
            }
        }
    }
...
int main(){
    ...
    convolutional_2D_constant_kernel<<<Blocks, Threads>>>(d_N, d_P, Width);
    ...
    verify_result(N.data(), M.data(), P.data(), Width);
